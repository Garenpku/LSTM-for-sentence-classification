...
class batch_generator():
    def __init__(self, raw, label, dictionary, batch_size):
        self.raw_text = [line.split(' ') for line in raw]
        self.max_len = max([len(line) for line in self.raw_text])
        self.dictionary = dictionary
        self.train_x = [[np.array(dictionary[word]).astype(np.float32) for word in line] for line in self.raw_text]
        for i in range(len(self.train_x)):
            for _ in range(len(self.train_x[i]), self.max_len):
                self.train_x[i].append(np.array(embedding_size * [0]).astype(np.float32))
...
def build_dataset(raw_text):
    ...
    for text in raw_text:
        cnt += 1
        if cnt % 10000 == 0:
            print(cnt, len(dictionary))
        for word in text.split(' '):
            dictionary[word] = np.random.rand(embedding_size) * 2 - 1
    pre_train = {}
    print(len(dictionary))
    count = 0
    for line in f:
        info = line[:-2].split(' ')
        if info[0] not in dictionary:
            continue
        else:
            dictionary[info[0]] = info[1:]
            count += 1
    print(len(dictionary) - count, "words not in dictionary")
    f.close()
    return dictionary, embedding_size

split_point = int(len(raw_text) * train_test_ratio)
dictionary, embedding_size = build_dataset(raw_text)
vocab_size = len(dictionary)
print("dataset building finished.")

batch = batch_generator(raw_text[:split_point], label[:split_point], dictionary, batch_size)
batch_test = batch_generator(raw_text[split_point:], label[split_point:], dictionary, batch_size)
...
# tf Graph input
x = tf.placeholder("float", [None, batch.max_len, embedding_size])
